...
model_select = Select(
    id="Model",
    label="select - Model",
    values=["openai-community/gpt2", "google/gemma-2b-it", "openai-community/gpt2-xl", "microsoft/Phi-3-vision-128k-instruct"],
    initial_index=0,
)
...
query_type = Select(
    ...
    values=["detailed_prompt","short_prompt", "summary_prompt", "explanation_prompt", "opinion_prompt", "instruction_prompt"],
    initial_index=0,
)
pdf_select = Select(
    id="Pdf",
...
pc_api = TextInput(
    id="pc_key",
    label="Enter your Pinecone API key",
    placeholder="sk-..."
)
...
async def start():
    ...
    cl.user_session.set("paths", {"slot-1": "", "slot-2": "", "slot-3": "", "slot-4": "", "slot-5": ""})
...
async def on_message(msg: cl.Message):
    ...
    if msg.content == ".data":
        ...
        ans = obj.chainlit_prompt(msg.content, cl.user_session.get("task"), cl.user_session.get("pc"), cl.user_session.get("hf"), cl.user_session.get("slot"), cl.user_session.get("model"))
        if ans:
